import { X, Play, Download, ExternalLink, Clock, Eye, ChevronLeft, Tag, Star } from "lucide-react";
import { Dialog, DialogContent } from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { ScrollArea } from "@/components/ui/scroll-area";
import type { Movie, Series, Episode } from "@/types/movie";
...
// Mock cast avatars - in production these would come from the API
const mockCastAvatars = [
      ...
      <DialogContent className="w-full max-w-[100vw] md:max-w-5xl h-[100dvh] md:h-auto md:max-h-[90vh] p-0 bg-transparent border-0 overflow-hidden shadow-none rounded-none md:rounded-3xl animate-in fade-in-0 zoom-in-95 duration-300">
        ...
        <div className="relative h-full md:h-auto md:rounded-3xl overflow-hidden">
          ...
          <div className="absolute inset-0">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-b from-card/90 via-card/95 to-card" />
          </div>

          <ScrollArea className="h-[100dvh] md:max-h-[90vh]">
            <div className="relative min-h-[100dvh] md:min-h-0">
              {/* Desktop Layout: Side by side */}
              <div className="hidden md:grid md:grid-cols-[400px,1fr] gap-8 p-8">
                {/* Left: Poster Card */}
                  ...
                  {movie.stars && movie.stars.length > 0 && (
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-6">
                        {movie.stars.slice(0, 4).map((star, index) => (
                          <div key={star} className="flex flex-col items-center gap-2 group cursor-pointer">
                            <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-border/30 shadow-lg transition-all duration-300 group-hover:border-primary/50 group-hover:scale-105 group-hover:shadow-xl">
                              <img
                                src={mockCastAvatars[index % mockCastAvatars.length]}
                                alt={star}
                  ...
                  {movie.stars && movie.stars.length > 0 && (
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-4 overflow-x-auto pb-2 -mx-5 px-5 scrollbar-none">
                        {movie.stars.slice(0, 5).map((star, index) => (
                          <div key={star} className="flex flex-col items-center gap-2 flex-shrink-0">
